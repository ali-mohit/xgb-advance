...
// NewConnNetViaCookieHex is just like NewConnNet and
...
func NewConnNetViaCookieHex(netConn net.Conn, cookieHex string) (*Conn, error) {
	c := &Conn{}

	// Connect using the provided network connection and hex-encoded authentication,
	// then load the initial Setup info.
	err := c.connectNetViaCookieHex(netConn, cookieHex)
