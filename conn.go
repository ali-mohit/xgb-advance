...
// connectNetViaCookieHex initializes the X connection using an existing net.Conn and a hex-encoded
...
func (c *Conn) connectNetViaCookieHex(netConn net.Conn, cookieHex string) error {
	c.conn = netConn
...
func (c *Conn) postConnect(cookieHex string) error {
	...
	if cookieHex == "" {
		...
		authName, authData, err = readAuthorityUsingFromCookieHex(cookieHex)
	}
